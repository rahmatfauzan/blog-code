...
// Props otomatis dari Next.js untuk akses URL params
...
export default async function ExplorePage({ searchParams }: ExplorePageProps) {
  ...
  return (
    <div className="container mx-auto max-w-7xl px-4 py-10 pb-24 min-h-screen">
      ...
      <SearchBar basePath="/my-snippets" placeholder="Cari snippet kamu..." />
        ...
        {snippets && snippets.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {snippets.map((snippet) => (
              // @ts-ignore - Tipe data sudah dihandle di query
              <SnippetCard key={snippet.id} snippet={snippet} />
            ))}
          </div>
        ) : (
          // Tampilan Kosong (Empty State)
          <div className="flex flex-col items-center justify-center py-20 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-900/50 animate-in fade-in zoom-in duration-300">
